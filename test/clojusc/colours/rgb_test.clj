...
(deftest test-rgb-colour-creation
  (testing "RGB foreground colour creation"
    (let [orange (rgb/rgb-colour 255 128 0)]
      (is (= 255 (:r orange)))
      (is (= 128 (:g orange)))
      (is (= 0 (:b orange)))
  ...
  (testing "RGB background colour creation"
    (let [orange-bg (rgb/rgb-bg-colour 255 128 0)]
      (is (= 255 (:r orange-bg)))
      (is (= 128 (:g orange-bg)))
      (is (= 0 (:b orange-bg)))
  ...
  (testing "RGB colour with no-colour flag"
    (let [orange (rgb/rgb-colour 255 128 0 true)]
      (is (true? (:no-colour? orange))))))

(deftest test-rgb-validation
  (testing "Valid RGB values"
    (is (rgb/rgb-colour 0 0 0))
    (is (rgb/rgb-colour 255 255 255))
    (is (rgb/rgb-colour 128 64 32)))
  
  (testing "Invalid RGB values throw errors"
    (is (thrown? AssertionError (rgb/rgb-colour -1 0 0)))
    (is (thrown? AssertionError (rgb/rgb-colour 256 0 0)))
    (is (thrown? AssertionError (rgb/rgb-colour 0 -1 0)))
    (is (thrown? AssertionError (rgb/rgb-colour 0 256 0)))
    (is (thrown? AssertionError (rgb/rgb-colour 0 0 -1)))
    (is (thrown? AssertionError (rgb/rgb-colour 0 0 256)))))
...
(deftest test-rgb-colourable
  ...
  (testing "Strip colours from RGB coloured text"
    (let [orange (rgb/rgb-colour 255 128 0)
      ...
      (is (= "test" (ansi/strip-colours orange coloured-text))))))